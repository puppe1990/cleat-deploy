...
| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/accounts/scope_test.exs` | builds scope from user+tenant; requires both |
...
| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/accounts/tenant_test.exs` | changeset requires name+slug |
| `test/cleat_deploy/accounts/membership_test.exs` | user+tenant+role unique |
| `test/cleat_deploy/accounts_test.exs` | `register_user_with_tenant/1` creates user, tenant, owner membership |
| `test/cleat_deploy/accounts_test.exs` | `get_scope_for_user/1` returns scope with tenant |

### Phase 2 — Migrations & backfill

| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/tenancy_migration_test.exs` | servers/apps have `tenant_id` NOT NULL after migration |
| `test/cleat_deploy/repo/seeds_test.exs` | seeds create `matheus.puppe@gmail.com` + trip-planner app under tenant |

### Phase 3 — Context tenancy (core)

| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/servers_test.exs` | `list_servers(scope)` returns only tenant servers |
| `test/cleat_deploy/servers_test.exs` | `get_server!(scope, id)` raises for other tenant's id |
| `test/cleat_deploy/servers_test.exs` | `create_server(scope, attrs)` sets `tenant_id` from scope |
| `test/cleat_deploy/apps_test.exs` | same isolation for apps |
| `test/cleat_deploy/apps_test.exs` | slug unique per tenant, duplicate allowed across tenants |
| `test/cleat_deploy/deployments_test.exs` | `for_app(scope, app)` rejects cross-tenant app |
| `test/cleat_deploy/deployments_test.exs` | `enqueue(scope, app)` only for owned app |

### Phase 4 — Auth & HTTP

| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy_web/user_auth_test.exs` | generated by `phx.gen.auth` |
| `test/cleat_deploy_web/controllers/user_session_controller_test.exs` | login/logout |
| `test/cleat_deploy_web/live/dashboard_live_test.exs` | unauthenticated → redirect login |
| `test/cleat_deploy_web/live/dashboard_live_test.exs` | authenticated sees only tenant apps |
| `test/cleat_deploy_web/live/server_live_test.exs` | cannot create server when logged out |
| `test/cleat_deploy_web/live/app_live_test.exs` | deploy button queues job for scoped app |
| `test/cleat_deploy_web/controllers/github_webhook_controller_test.exs` | still works without session |

### Phase 5 — Turso config

| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/config/turso_config_test.exs` | prod env vars produce LibSql adapter config (no live connection) |

### Phase 6 — Integration (tag `@tag :integration`)

| Test file | Tests |
|-----------|-------|
| `test/cleat_deploy/integration/tenancy_deploy_test.exs` | matheus scope sees trip-planner; other user does not |
...
- [ ] **Step 2:** Run `mix test test/cleat_deploy_web/user_auth_test.exs` — GREEN
- [ ] **Step 3:** Commit `feat: add phx.gen.auth scaffold`

### Task 2: Tenant schema

**Files:**
- Create: `lib/cleat_deploy/accounts/tenant.ex`
- Create: `lib/cleat_deploy/accounts/tenant_membership.ex`
- Create: migration `create_tenants`
- Test: `test/cleat_deploy/accounts/tenant_test.exs`
...
**Files:**
- Create: `lib/cleat_deploy/accounts/scope.ex`
- Modify: `lib/cleat_deploy/accounts.ex`
- Test: `test/cleat_deploy/accounts_test.exs`
...
- Test: `test/cleat_deploy/repo/seeds_test.exs`
...
- Test: `test/cleat_deploy/config/turso_config_test.exs`
